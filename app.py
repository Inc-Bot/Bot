...
# --- Конфигурация ---
API_TOKEN = '8070912034:AAGjUSq_GpmE5-1-eqHlgnnQhAdnohDagEU'       
...
DEFAULT_SETTINGS = {
    ...
    'pass_exp_requirements': { # Лимит очков для каждого уровня
        ...
        46: 50000, 47: 50000, 48: 50000, 49: 50000, 50: 100000
    },
    'pass_rewards': { # Награды за уровни
        ...
        16: {'type': 'normal_money', 'amount': 100000},
        17: {'type': 'normal_money', 'amount': 100000},
        18: {'type': 'normal_money', 'amount': 100000},
        19: {'type': 'normal_money', 'amount': 100000},
        20: {'type': 'normal_money', 'amount': 100000},
        21: {'type': 'normal_money', 'amount': 100000},
        22: {'type': 'normal_money', 'amount': 100000},
        23: {'type': 'normal_money', 'amount': 100000},
        24: {'type': 'normal_money', 'amount': 100000},
        25: {'type': 'galactic_money', 'amount': 1},  # 1 галактическая монета
        26: {'type': 'normal_money', 'amount': 500000},
        27: {'type': 'normal_money', 'amount': 500000},
        28: {'type': 'normal_money', 'amount': 500000},
        29: {'type': 'normal_money', 'amount': 500000},
        30: {'type': 'normal_money', 'amount': 500000},
        31: {'type': 'normal_money', 'amount': 500000},
        32: {'type': 'normal_money', 'amount': 500000},
        33: {'type': 'normal_money', 'amount': 500000},
        34: {'type': 'normal_money', 'amount': 500000},
        35: {'type': 'normal_money', 'amount': 1000000},
        36: {'type': 'normal_money', 'amount': 1000000},
        37: {'type': 'normal_money', 'amount': 1000000},
        38: {'type': 'normal_money', 'amount': 1000000},
        39: {'type': 'normal_money', 'amount': 1000000},
        40: {'type': 'normal_money', 'amount': 2500000},  
        41: {'type': 'normal_money', 'amount': 1500000},
        42: {'type': 'normal_money', 'amount': 1500000},
        43: {'type': 'normal_money', 'amount': 1500000},
        44: {'type': 'normal_money', 'amount': 1500000},
        ...
        50: {'type': 'case', 'case_name': 'mythical'}  # PRO подписка навсегда
    },
        ...
        if not user:
            ...
            for i, col in enumerate(cursor.description):
                ...
                if key in ['theft_last_use', 'last_login_date', 'last_transfer_reset', 'last_special_coin_payout',
                                           ...
                                           'last_mine_reset', 'last_fish_reset', 'last_hunt_reset', 'last_herb_search', 'luck_potion_end_time', 'last_restaurant_payout', 'pro_subscription_until', 'last_title_special_payout'] and value is not None:
                    try:
...
def register_tutorial_handlers(
    ...
    def _finish_training(user_id: int):
        return update_user_data(user_id, {'tutorial_completed': 1})
    ...
    def _build_final_keyboard(bot_username: str, user_id: int) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="Чат", url="https://t.me/IncBot_Chat")],
    ...
    async def tutorial_unknown(message: Message, state: FSMContext):
        ...
        state_map = {
            ...
            TutorialStates.step6.state: (6, ('собирать грибы',)),
        }
        ...
        if allowed_commands:
            ...
            return
        elif step_number == 7 and text.startswith('собирать грибы'):
            return
...
async def get_pass_info(user_id: int) -> str:
    ...
    season_end = datetime.datetime(2026, 6, 3, 15, 0)
...
Вы получили всё награды, включая <b>PRO подписку навсегда</b>! 👑{season_text}
"""
...
async def command_titles_handler(message: Message):
    ...
    for title in titles:
        ...
        if title != active_title:
            keyboard.append([InlineKeyboardButton(text=f"Выбрать: {title}", callback_data=f"select_title:{title}")])
        else:
            keyboard.append([InlineKeyboardButton(text=f"✅ Активен: {title}", callback_data=f"deselect_title:{title}")])
    
    # Добавляем кнопку для удаления титула
    keyboard.append([InlineKeyboardButton(text="❌ Убрать титул из профиля", callback_data="deselect_title:None")])
...
async def select_title_callback(call: CallbackQuery):
    """Выбирает титул"""
    title_name = call.data.replace("select_title:", "")
...
async def deselect_title_callback(call: CallbackQuery):
    ...
    title_data = call.data.replace("deselect_title:", "")
    user_id = call.from_user.id
...
@dp.message(F.text.lower().startswith("собирать грибы"))
async def search_herb_event(message: Message):
    ...
    if last_search and (now - last_search) < datetime.timedelta(hours=1):
        ...
        await message.answer(
            f"⏳ Можно собирать грибы снова через <b>{minutes}</b> мин <b>{seconds}</b> сек.",
            parse_mode="HTML"
        )
        return
    ...
    await message.answer(
        f"🍄‍🟫 Вы отправились собирать грибы и получаете <b>{exp_amount}</b> очков Inc Pass.",
        parse_mode="HTML"
    )
...
async def command_upgrade_pickaxe_handler(message: Message):
    user = await get_user_data(message.from_user.id)
    current_level = user.get('mining_pickaxe_level', 1)
    costs_for_next_level = {1: 10000, 2: 100000, 3: 200000, 4: 300000, 5: 400000, 6: 450000, 7: 750000, 8: 1000000, 9: 12000000, 10: 15000000, 11: 19500000, 12: 26325000, 13: 3685050, 14: 5528025, 15: 8560879, 16: 13710006, 17: 23030010}
...
async def check_daily_login(user_id: int, manual_call=False, message: Message | None = None):
    ...
    # ✨ Особые монеты за титулы
    titles = user.get('titles', [])
    if titles:
        last_title_payout = user.get('last_title_special_payout')
        if last_title_payout is None or (now - last_title_payout) >= datetime.timedelta(days=1):
            special_coins = len(titles)
            user['balance_special'] = user.get('balance_special', 0) + special_coins
            user['last_title_special_payout'] = now
            reward_text += f"\n\n✨ За титулы: <b>+{special_coins}</b> особых монет"
            # Сохраняем изменения
            await update_user_data(user_id, user)
...
async def command_donate_handler(message: Message):
    text = (
        ...
        "<a href='http://t.me/wladonchek5'>тык</a>\n"
...
async def show_game_commands(call: CallbackQuery):
    help_text = (
        ...
        "<b>🏹 Охота</b> - Охотиться.\n"
        "<b>🍄‍🟫 Собирать грибы</b> - Собирать грибы и получать Inc Pass.\n"
...
async def show_pass_help(call: CallbackQuery):
    help_text = (
        ...
        "Ивент <b>«Собирать грибы»</b> сделан специально для Inc Pass: в нём вы получаете пасс-очки.\n\n"
        "За каждого нового приглашённого друга реферер получает <b>+1,000 очков Inc Pass</b> и от <b>100,000</b> монет.\n"
        "При достижении уровней вы получаете разные награды: монеты, особые монеты, кейсы и особый титул.\n\n"
...
async def show_donate_commands(call: CallbackQuery):
    help_text = (
        ...
        " \n"
        "Для покупки Донат кейса или Денежного кейса пишите @wladonchek5.\n"
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="main_menu")]
    ])
